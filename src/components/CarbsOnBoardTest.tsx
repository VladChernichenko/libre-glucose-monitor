...
export const CarbsOnBoardTest: React.FC = () => {
  ...
  const handleCalculate = async () => {
    ...
    try {
      const result = await testApiService.calculateCOB(userId, {
        carbs,
  ...
  const handleGetStatus = async () => {
    ...
    try {
      const result = await testApiService.getCOBStatus(userId);
      setCobStatus(result);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Unknown error occurred');
    } finally {
  ...
  const handleGetTimeline = async () => {
    ...
    try {
      const result = await testApiService.getCOBTimeline(userId);
